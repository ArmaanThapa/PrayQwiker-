...
//----------------------------------------
function resendVerificationCode(val) {
		var emailid = document.getElementById("remailid").value=val;
		console.log("Email is :" +emailid);
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				//console.log("in if ready state condition");
				document.getElementById("resend_email_div").innerHTML = this.responseText;
			}
			else {
				document.getElementById("resend_email_div").innerHTML = "";
			}
		};
		xhttp.open("GET", "<%=request.getContextPath()%>/ajaxLoginEmailValidator?remailid=" + remailid, true);
		xhttp.send();
	}
	//-------------
	function submitResendVerificationCode(){
		var emailid = document.getElementById("remailid").value;
		console.log("Email in submit :" +emailid);
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				//console.log("in if ready state condition");
		...
		xhttp.open("GET", "<%=request.getContextPath()%>/ajaxResendVerificationCode?emailid=" + emailid, true);
		xhttp.send();
	}
//------------------------------------------
...
//--------------------
	...
	function submitVerificationCode(){
		...
		//alert("code is  -- >  "  +vcode)
		console.log("vcode before submit ---> " + vcode);
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				console.log("in if ready state condition");
		...
		xhttp.open("GET", "<%=request.getContextPath()%>/verifyCode?code="+vcode, true);
		xhttp.send();
...
<body>
<%-- <div align="center" style="color: red;background-color: maroon; font-size: medium; border-width: thin;">${loginError}</div>
<div align="center" style="color: red;background-color: maroon; font-size: medium; border-width: thin;">${firstnameError}${lastnameError}${emailError}${mobileError}${passwordError}${confirmpasswordError}</div>
 --%>
<div align="center" style="color: red;background-color: orange; font-size: medium; border-width: thin;"> ${verifyAccountError }</div>
<div align="center" style="color: red;background-color:orange; font-size: medium; border-width: thin;">${vcodeError}</div>
<div align="center" style="color: green;background-color: orange;font-size: medium;"><b>${resendSuccess}</b></div>
<div align="center" style="color: green;background-color: orange;font-size: medium;">${successMessage}&nbsp;&nbsp;&nbsp;&nbsp;

<a href="#" data-toggle="modal" data-target="#verifycodeModal">Verify Account</a>
&nbsp;&nbsp;&nbsp;&nbsp;
<a href="#" data-toggle="modal" data-target="#resendVerificationCodeModal">Resend Verification Code</a>
</div>
						...
						<div id="login">
							...
							<div class="tab-content">
								...
								<div id="signup" class="tab-pane fade text-center">
									<br>
									<form action="<%=request.getContextPath()%>/usersignup" method="post" name="form">
			...
			<div class="col-md-12 col-lg-offset-1">
				...
				<div class="mainmenu pull-left">
					<ul class="nav navbar-nav collapse navbar-collapse">
						<li><a href="index.html" class="active">Electronics</a></li>
							...
							<div class="single-products">
								...
								<div class="productinfo text-center">
									<a
										...
										<img
										src="<%=request.getContextPath()%>/resources/images/uploaded-images/${pobj.fileName}"
		...
		<div class="modal-dialog modal-sm">
			...
			<div class="modal-content">
				<div class="modal-body text-center">
					<form action="<%=request.getContextPath()%>/forgottenPassword">
						...
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}" /> <br> <input type="submit"
							class="btn btn-default" value="Submit" data-toggle="modal"
							data-target="#messageModal" onClick="userForgottenPassword()" /><br>
						<br>
					</form>
				</div>
			</div>
...
<div id="verifycodeModal" class="modal fade" role="dialog">
			...
			<div class="modal-content">
				<div class="modal-body text-center">
					<form action="<%=request.getContextPath()%>/verifyCode" method="post">
						...
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}"/>
							<!-- <h6><a href="#" data-toggle="modal" data-target="#resendVerificationCodeModal"> Resend Verification Code</a></h6> --> 
							<input type="submit"
							class="btn btn-default" value="Submit" onClick="submitVerificationCode()"/><br> <br>
					</form> <!--  -->
				</div>
			</div>
		</div>
	...
	<!-- ---------------------------------------/verifycodeModal------- -->
	<!-- ---------------------------------------resend code------- -->
	<div id="resendVerificationCodeModal" class="modal fade" role="dialog">
		<div class="modal-dialog modal-sm">
			<!-- Modal content-->
			<div class="modal-content">
				<div class="modal-body text-center">
				<%-- <%=request.getContextPath()%>/ajaxResendVerificationCode <%=request.getContextPath()%>--%>
					<form action="<%=request.getContextPath()%>/ajaxResendVerificationCode">
						<h5>${message}</h5>
						<h3>Enter Your Email Id</h3>
						<input type="text" name="remailid" id="remailid"
							 onchange="resendVerificationCode(this.value)"
							placeholder="Enter Email Id" required="required" />
						<div id="resend_email_div" style="color: red"></div>
						<h4 style="color: red;">${verifyFail}</h4>
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}" /> <br>
							<input type="submit"
							class="btn btn-default" value="Resend" onClick="submitResendVerificationCode()"/><br> <br>
					</form>
				</div>
			</div>
		</div>
